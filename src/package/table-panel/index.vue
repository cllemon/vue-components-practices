...
export default {
  ...
  computed: {
    ...
    isPresenceSelectionColumnProps() {
      return (
        this.isObject(this.selectionColumnProps) && Object.keys(this.selectionColumnProps).length
      );
    },
  },
  ...
  methods: {
    ...
    formatter(row, rowRule) {
      return rowRule.formatter ? rowRule.formatter(row[rowRule.prop], row) : row[rowRule.prop];
    },
